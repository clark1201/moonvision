...
my $statement = qq{select a.id as accountid, b.id as bookid, b.book_time, b.book_am_pm, b.detail, b.create_time from booking as b inner join account as a where a.tel=? and a.id=?};
...
while (@data = $sth->fetchrow_array()) {
  ...
  $str = $str."{\"account_id\":\"$account_id\", \"bookid\":\"$bookid\", \"book_time\":\"$book_time\", \"book_am_pm\":\"$book_am_pm\", \"detail\":\"$detail\", \"create_time\":\"$create_time\"},";
}
